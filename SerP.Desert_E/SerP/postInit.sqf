...
if (!isDedicated) then {
	...
	[] call compile preprocessFileLineNumbers "SerP\scripts\briefing.sqf";
	
	if (isMultiplayer) then { enableRadio false; };
	[["AllVehicles"], [ace_sys_interaction_key], 2, ["SerP\scripts\interactionMenu.sqf", "main"]] call CBA_ui_fnc_add;
	[["player"], [ace_sys_interaction_key_self], 2, ["SerP\scripts\selfInteractionMenu.sqf", "main"]] call CBA_ui_fnc_add;

	call compile preprocessFileLineNumbers "SerP\scripts\spectator.sqf";
	["Client postInit loaded."] call FNC(Status);
};

//testing
if (isServer && !isDedicated) then {
	[] call compile preprocessFileLineNumbers "SerP\scripts\template_test.sqf";
};