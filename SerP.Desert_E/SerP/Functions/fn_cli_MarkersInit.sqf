...
#define getParamNumber(VAR1) getNumber(missionConfigFile >> "SerP_const" >> #VAR1)
#define getParamTextCompile(VAR1) (call compile getText(missionConfigFile >> "SerP_const" >> #VAR1))
...
_playerPos = PGVAR(SpawnPosition,nil);
_playerGroup = group player;
if (isNil "_playerPos") then {
	_leaderPos = OGVAR(leader _playerGroup,SpawnPosition,nil);
	if (isNil "_leaderPos") then {
		_playerPos = getPosASL (leader _playerGroup);
	} else {
		_playerPos = _leaderPos;
	};
...
_spawnZoneType = ["spawnZoneType", 0] call SerP_blnd_fnc_GetParam; // Get spawn zone display mode

private ["_spawnZonePos", "_spawnZoneSize"];

if (_spawnZoneType == 0) then {
	_spawnZone = call compile preprocessFileLineNumbers "SerP\Scripts\cli_spawnZones.sqf";
...
for "_i" from 0 to ((count _allGroups) - 1) do {
	...
	if (_side == _playerSide) then {
		_pos = OGVAR(_leader,SpawnPosition,nil);
		if (isNil "_pos") then {
...
player createDiaryRecord ["Diary", [localize "STR_serp_Settings", localize "STR_serp_markersControl"]];
