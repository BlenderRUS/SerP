...
if ((count _currPair) == 2) then {
	if (player getVariable ["ACE_PB_Result", -1] == 0) exitWith {
		hint (localize "STR_SerP_OpticsBusy");
	};
	[5,[localize "STR_SerP_OpticsChanging"],true,true] spawn ace_progressbar;
	...
	if (_result == 1) then {
		...
		switch true do {
			case ((_currPair select 0) in _weapons): {
				...
				hint format[localize "STR_SerP_OpticsChangedTo", [_weap_1, "CfgWeapons"] call FNC(GetName), [_weap_2, "CfgWeapons"] call FNC(GetName)];
			};
			...
			default {hint (localize "STR_SerP_OpticsNotFound")};
		};
	} else {
		switch _result do {
			case 2: { hint (localize "STR_SerP_OpticsMoved") };
			default { hint "Optics change Failed!" };
		};
	};
} else {
	hint (localize "STR_SerP_OpticsNotFound");
};