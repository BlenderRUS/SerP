...
if (_version==0) then { _version = getText(missionConfigFile >> "SerP_version") } else { _version = "v"+str(_version) };
_edition = getText (missionConfigFile >> "SerP_edition");
if (_edition == "") then { _edition = "ERROR!" };
_cred = player createDiaryRecord ["diary", [localize "credits_title",format ["%1 <br/>SerP %2 (Редакция: %3)<br/>Автор платформы: conKORD<br/>Переработка и поддержка: Blender",localize "credits",_version,_edition]]];
